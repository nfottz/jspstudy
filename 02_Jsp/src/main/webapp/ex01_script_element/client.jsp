...
<body>
	...
	<%--
		...
		2. JavaScript 변수를 Java에서 사용할 수 없다..ㅠ
	 --%>
	 ...
	 <%-- 연습1. getNumber() 메소드의 반환 결과가 5 이상이면 "큰 수" 아니면 "작은 수" 출력하기 --%>
	 <%
	 if(getNumber() >= 5) { %>
	 	<h1>큰 수</h1>
	 <% } else { %>
	 ...
	 <%
	 int num = getNumber();
	 String answer = (num >= 5) ? "큰 수" : "작은 수";
	 %>
	 <h1><%=num %>은(는) <%=answer %>입니다!</h1>
	 
	 <%-- 연습2. 1월 ~ 12월이 표시되는 <select> 태그 작성하기 --%>
	 <select name="month">
	 	<% for(int m = 1; m <= 12; m++){ %>
