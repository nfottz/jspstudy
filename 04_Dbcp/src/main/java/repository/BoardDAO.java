...
public class BoardDAO {
	...
	private BoardDAO() {
		...
		try {
			...
			Context envContext = (Context)context.lookup("java:comp/env");	// 톰캣환경
	...
	public List<BoardDTO> selectBoardList(){
		...
		try {
			...
			// 5. PreparedStatment 객체를 이용해 쿼리문 실행
			// SELECT문 실행은 executeQuery 메소드로 한다. (-> ResultSet 반환됨)
			rs = ps.executeQuery();
			
			// 6. ResultSet 객체(결과 집합)를 이용하여 ArrayList 만들기
			while(rs.next()) {
				
				// 1) Board 테이블의 결과 행(ROW)을 읽는다.
	...
	public BoardDTO selectBoardByNo(int board_no) {
		...
		try {
			...
			// 5. 쿼리문에 변수 값 전달하기
			ps.setInt(1, board_no);		// 1번째 물음표에 board_no(매개변수) 전달
			
			// 6. PreparedStatment 객체를 이용해 쿼리문 실행
			// SELECT문 실행은 executeQuery 메소드로 한다. (-> ResultSet 반환됨)
			rs = ps.executeQuery();
			
			// 7. ResultSet 객체(결과 집합)를 이용해서 BoardDTO 만들기
			if(rs.next()) {
	...
	public int insertBoard(BoardDTO board) {
		...
		try {
			...
			// 6. PreparedStatment 객체를 이용해 쿼리문 실행
			// INSERT문 실행은 executeUpdate 메소드로 한다. (-> 0 또는 1 반환됨)
	...
	public int updateBoard(BoardDTO board) {
		...
		try {
			...
			// 6. PreparedStatment 객체를 이용해 쿼리문 실행
			// UPDATE문 실행은 executeUpdate 메소드로 한다.
	...
	public int deleteBoard(int board_no) {
		...
		try {
			...
			// 6. PreparedStatment 객체를 이용해 쿼리문 실행
			// DELETE문 실행은 executeUpdate 메소드로 한다.
